...
public class MemberServiceImpl implements MemberService{
	...
	public int insertMember(Member vo) throws Exception{
		...
		mDao.createAuthkey(vo.getEmail(), key); // 인증키 DB저장
		MailHandler sendMail = new MailHandler(mailSender);
		sendMail.setSubject("인증 이메일 테스트입니다.");
		sendMail.setText(
				new StringBuffer().append("<h1>메일인증</h1>").append("<a href='http://localhost:8090/bts/emailConfirm?email=").append(vo.getEmail()).append("&authkey=").append(key).append("' targer='blenk'>이메일 인증 확인</a>").toString());
